...
public class DateUtils {
    ...
    public static String getUTCTimeStr() {
        StringBuilder uctTimeBuffer = new StringBuilder();
        // 1、取得本地时间：
        ...
        uctTimeBuffer.append(year).append("-").append(month).append("-").append(day);
        uctTimeBuffer.append(" ").append(hour).append(":").append(minute);
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
            sdf.parse(uctTimeBuffer.toString());
            return uctTimeBuffer.toString();
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return null;
